...
The latest artifact comparison shows that not every `prepare_ota -> ota_monitor` failure is the same class. Pipeline `#835` looked at first like a clean boundary-only reproduction of "stale OTA state", but the timeline proves it was contaminated by another pipeline touching the same physical set and AWS Thing. Slot #1 on pipeline `#835` built `v1=946 / v2=947`, finished `prepare_ota` at `2026-03-14 23:21:43 JST`, and created OTA update `rx72n-ota-1773498142` at `23:21:44`. Two seconds later, pipeline `#834` started `prepare_ota` on the same `rx72n-01` set (`23:21:46 -> 23:23:53`), which re-flashed the board back to its own `v1=944`. Pipeline `#834` then ran `ota_create_job` at `23:23:54`, deleted `#835`'s OTA update `1773498142` as stale, and created `rx72n-ota-1773498272` for its own `v2=945`. When `#835` finally entered `ota_monitor` at `23:23:56`, it immediately saw current firmware `2.0.944`, received AWS job ID `AFR_OTA-rx72n-ota-1773498272`, and rebooted into `2.0.945`. In other words, `#835` did not consume its own job at all; it consumed `#834`'s newer full-pipeline job after the board had already been re-prepared by `#834`.

The same overlap pattern appears in the original full-pipeline failures `#814` and `#815`. Pipeline `#814` built `v1=930 / v2=931` and created OTA update `rx72n-ota-1773487419` with no stale matches. Pipeline `#815`, started a few minutes later on the same slot, built `v1=932 / v2=933`, then its `ota_create_job` explicitly matched and deleted `rx72n-ota-1773487419` before creating `rx72n-ota-1773487560`. Crucially, `#814`'s `ota_monitor` expected build `931` but already detected current firmware `2.0.932` at monitor start, which means the board had been re-flashed by `#815` before `#814` began monitoring. This is the same structural failure as `#834/#835`: the device, Thing name, and static S3 key `ota/rx72n-01/ota_v2.rsu` were reused across overlapping pipelines.

This does not explain every unstable run. Pipeline `#786` still failed without the same overlap signature because pipeline `#784` had already finished before `#786` started, and no neighboring legacy OTA pipeline was active on slot #1 during `#786`'s boundary window. So the current picture is two-layered: `#814/#815` and `#834/#835` are now explained by cross-pipeline interference, while `#786` remains the clean non-overlap failure that should be used when hunting the remaining runtime/hardware/reset issue.

The immediate practical takeaway is that the current locking is too narrow for legacy OTA analysis. `resource_group` and `acquire_pi_device_lock.sh` protect each Raspberry Pi job separately, but they do not reserve the device across the full `prepare_ota -> ota_create_job -> ota_monitor -> ota_finalize` boundary. That leaves a gap where a later pipeline can reflash the board or delete/replace the AWS OTA update before the earlier pipeline starts monitoring. Future repro runs for Issue `#18` need to avoid concurrent pipelines on the same `DEVICE_ID` / Thing name, or introduce a reservation mechanism that spans the whole OTA boundary rather than individual jobs.

artifact 比較を進めた結果、`prepare_ota -> ota_monitor` failure は全部同じ種類ではないことが分かった。pipeline `#835` は当初「stale OTA state の clean な境界再現」に見えていたが、実際には同じ実機 set と同じ AWS Thing を触る別 pipeline が途中で割り込んでいた。slot #1 の `#835` は `v1=946 / v2=947` を build し、`2026-03-14 23:21:43 JST` に `prepare_ota` を終え、`23:21:44` に OTA update `rx72n-ota-1773498142` を作成している。ところがその 2 秒後、pipeline `#834` が同じ `rx72n-01` で `prepare_ota` を開始し (`23:21:46 -> 23:23:53`)、board を自分の `v1=944` へ書き戻した。さらに `#834` の `ota_create_job` は `23:23:54` に `#835` の OTA update `1773498142` を stale として削除し、自分の `v2=945` 用に `rx72n-ota-1773498272` を作り直している。結果として `#835` の `ota_monitor` は `23:23:56` 開始時点で current firmware `2.0.944` を見ており、受信した AWS job ID も `AFR_OTA-rx72n-ota-1773498272`、最終 reboot 後 version も `2.0.945` だった。つまり `#835` は自分の job を処理したのではなく、後から割り込んだ `#834` の job を処理していた。
