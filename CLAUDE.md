...
| - | SD カード更新の CI/CD 完全自動化: UART ファイル転送コマンド + GUI ボタン操作コマンドの実装（ファームウェア変更） | Planned |
...
**serial_terminal_task の LCD タッチ依存 (Phase 5 で解消):**

`serial_terminal_task` は起動時に `ulTaskNotifyTake(pdTRUE, portMAX_DELAY)` で GUI 初期化完了通知を待つ。
元のコードでは `ID_SCREEN_00_Slots.c` の LCD タッチイベント (`PIDPRESSED`) で通知されるため、
LCD 画面を 1 回タッチするまで UART コマンドターミナルが一切動作しなかった。
CI/CD 自動テストではタッチ操作が不可能なため、`gui_task.c` の `APPW_CreateRoot()` 直後に
`xTaskNotifyGive(serial_terminal_task_handle)` を追加し、LCD タッチなしで起動するように変更した。
`serial_terminal_task` は SCI (UART) のみ使用し、LCD ウィンドウは参照しない (`#if 0` で無効化済み)。
...
### 2026-03-01: Phase 5 — 既存機能の動作検証（テストスクリプト・CI/CD 追加）

Added automated test scripts and CI/CD test stage for Phase 5 feature validation.
Fixed serial_terminal_task LCD touch dependency to enable CI/CD automated UART testing.

Phase 5 の動作検証のためのテストスクリプトと CI/CD テストステージを追加。
serial_terminal_task の LCD タッチ依存を解消し、CI/CD での UART 自動テストを実現。

**Firmware changes / ファームウェア変更:**
- `gui_task.c`: `APPW_CreateRoot()` 直後に `xTaskNotifyGive(serial_terminal_task_handle)` を追加
  - LCD タッチなしで serial_terminal_task が起動するように変更（CI/CD 対応）
...
- Phase 5 ステータス更新
- serial_terminal_task LCD タッチ依存・COM6 障害復旧・ポートロック問題の知見を追記
