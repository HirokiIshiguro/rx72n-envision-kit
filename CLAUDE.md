...
- ただし v3.07.00 は LCD 消灯バグがあるため運用では v3.04.00 を使用（Phase 2 トラブルシューティング #6 参照）
...
- 備考: v3.07.00 では aws_demos が約10分後に LCD 消灯するバグが発生（Phase 2 トラブルシューティング #6 参照）。v3.04.00 に戻して解決（MR !11）。Runner マシンには v3.04.00 と v3.07.00 の両方をインストール済み
...
**6. CC-RX v3.07.00 で aws_demos が約10分後に LCD 消灯（パイプライン #172 vs #177）**
- 症状: aws_demos ダッシュボード画面遷移後、約10分で LCD バックライト消灯。タッチ・リセットで復帰不能
- 切り分け:
  - `bin/boot_loader/v100_20200211/rx72n_boot_loader.mot`（v3.04.00 手動ビルド）→ rfp-cli で書き込み → 正常起動
  - Pipeline #172（v3.07.00 CI ビルド）→ ~10分で消灯、以後起動不能
  - Pipeline #177（v3.04.00 CI ビルド）→ 10分以上放置しても消灯せず正常動作
- 原因: CC-RX v3.07.00 のコード生成差異。ハードフォルトまたはスタックオーバーフローにより GPIO がリセットデフォルト（バックライト OFF）に戻ると推定
- 対策: `.cproject` のツールチェーンバージョンを v3.04.00 に戻す（MR !11）
- 備考: v3.07.00 の具体的なバグ箇所は未特定。将来的にルネサスに報告する場合はバイナリ差分解析が必要
