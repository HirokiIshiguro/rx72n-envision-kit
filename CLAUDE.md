...
- upstream リポジトリ https://github.com/renesas/rx72n-envision-kit は、2024 年以降ほぼ保守されていません。
- 本リポジトリは、既存ユーザーが更新を追いやすいよう GitHub fork 側にも mirror しています。
...
| 4 | FreeRTOS を最新 Renesas IoT reference implementation ([iot-reference-rx](https://github.com/renesas/iot-reference-rx)) に置換 | 進行中 (Phase 8b 第3次 段階1-4 完了 / 段階5 GUI/SD 再統合 残, 新規 Issue 要起票。旧 Issue #12 は 2026-03-15 に閉鎖) |
...
| フェーズ | 目標 | 状態 |
|-------|------|--------|
| 1 | e2studio ヘッドレスビルド（3プロジェクト） | 完了 |
| 2 | flash（rfp-cli）+ UART テスト自動化 | 完了 (MR !9) |
| 3 | UART ダウンロード後のフリーズ現象の解析・修正（原因: BANKSEL 未リセット、修正: `-erase-chip`） | 完了 |
| 4 | boot_loader の UART ダウンロードを COM6 (SCI2, 115200bps) から COM7 (SCI7 PMOD FTDI, 921600bps) に変更（要ファームウェア SCI ポート変更。ダウンロード時間短縮でデバッグ効率改善） | 完了 |
...
| 8 | FreeRTOS LTS 最新版適用（[iot-reference-rx](https://github.com/renesas/iot-reference-rx) 最新リリースタグ）。作業リポジトリ: [iot-reference-rx (GitLab)](https://shelty2.servegame.com/oss/import/github/renesas/iot-reference-rx)。CK-RX65N V1 で先行構築（Phase 8a）→ RX72N に移植（Phase 8b）の2段階アプローチ。詳細計画は [iot-reference-rx の CLAUDE.md](https://shelty2.servegame.com/oss/import/github/renesas/iot-reference-rx/-/blob/main/CLAUDE.md) を参照 | 進行中 (Phase 8b 第3次 段階1-4 完了, 段階5 GUI/SD は新規 Issue 要起票。旧 #12 は 2026-03-15 閉鎖) |
| 9 | AWS 接続を含む OTA テスト（1台、新 FW で再検証） | 完了 (MR !83, Pipeline #3123 で v3 OTA 全 7 jobs success) |
| 10 | AWS 接続を含むフリートプロビジョニング テスト（1台。iot-reference-rx の FP デモを活用） | 予定 |
| 11 | AWS 接続を含むセカンダリ MCU ファームウェアアップデート テスト（RX72N → FPB-RX140） | 予定 |
| 12 | OTA × 3 一斉テスト | 予定 |
| 13 | フリートプロビジョニング × 3 + 一斉 OTA テスト | 予定 |
| 14 | フリートプロビジョニング × 3 + セカンダリ MCU アップデート × 2 + 一斉 OTA テスト（フル構成） | 予定 |
| - | UART テストスクリプトの共通ライブラリ化（[mcu-test/uart](https://shelty2.servegame.com/oss/experiment/generic/scripts/python/mcu-test) へ切り出し、git submodule で各プロジェクトから参照） | 予定 |
| - | mot_to_rsu コンバータの共通部品化（git submodule で各プロジェクトから参照） | 完了 (MR !12) |
| - | AWS CLI / IoT Core ノウハウを `oss/experiment/cloud/aws/iot-core/claude` に export | 完了 (MR !1 on iot-core/claude) |
| - | SD カード更新の CI/CD 完全自動化: UART ファイル転送コマンド (`sdcard write`) + GUI ボタン操作コマンド (`touch`) の実装（ファームウェア変更） | 完了 (MR !20) |
| - | パイプライン条件分岐（`RUN_AWS_TESTS` / `RUN_SD_UPDATE_TEST` / `RUN_OTA_TEST` 変数で各テストを選択実行） | 完了 (MR !23) |
| - | BUTTON_03 タッチ問題: J-Link 実機デバッグで WM_NOTIFICATION_CLICKED 発火確認 | 予定 |
| - | Runner 分離: ビルド専用 (Windows) / 実機操作専用 (Raspberry Pi) に分けて並列度向上 | 完了 (MR !32) |
...
| 8b-4 | [#10](https://shelty2.servegame.com/oss/import/github/renesas/rx72n-envision-kit/-/issues/10) (closed) / [#46](https://shelty2.servegame.com/oss/import/github/renesas/rx72n-envision-kit/-/issues/46) (open, MR !83 マージ済みのため close 予定) | OTA を新 baseline 上で再検証 (第3次 段階4 で完了, MR !83) |
| 8b-5 | 新規 Issue 要起票 (旧 [#12](https://shelty2.servegame.com/oss/import/github/renesas/rx72n-envision-kit/-/issues/12) は 2026-03-15 closed) | GUI / SD update / Envision Kit 独自 UX を再統合 (次着手) |

**実行順:**
- 最初のターゲットは headless baseline の `build -> flash -> provision -> MQTT -> OTA`。
- GUI (`emWin` / AppWizard)、SD update UX、その他 Envision Kit 固有機能は後段で戻す。
- MCUboot 移行は別トラックとして扱い、初期 FreeRTOS baseline port へ混ぜない。

**目標構成:**
- 現行 repo はまだ legacy の lower-case tree (`projects/`, `vendors/`, `libraries/`, `freertos_kernel/`) を使っている。
- 目標 layout は `iot-reference-rx` に合わせた `Common/`, `Configuration/`, `Demos/`, `Middleware/`, `Projects/`, `Test/`。
- 初期 RX72N project 名は `aws_ether_rx72n_envision_kit` と `boot_loader_rx72n_envision_kit` を想定する。
- Phase 8b-1 では `phase8b/` を staging root にして、Windows 上の現行 tree を壊さずに新 layout を準備する。

**Windows ファイルシステム制約:**
- 本 repository は現在 Windows 上で `core.ignorecase=true` で運用している。
- `projects` -> `Projects` や `vendors` -> `Middleware` のような case-only rename は、temporary name を挟むか case-sensitive 環境で慎重に行う必要がある。
- このため Phase 8b-1 ではまず移行手順の文書化と準備にとどめ、破壊的な top-level rename はまだ行わない。

詳細メモは [`docs/phase8b-migration-plan.md`](docs/phase8b-migration-plan.md) に記録する。
...
### Phase 8b 事前確認: MCUboot + 最新 FreeRTOS 向け ROM 予算
...
| イメージ | ROMDATA | PROGRAM | Flash 相当合計 | 予算 | 余裕 |
|------|--------:|--------:|-----------------:|-------:|---------:|
...
| `RUN_V3_BUILD_ONLY` | `"false"` | v3 baseline の build-only gate (`build_v3_rx72n`) のみを実行するか (Phase 8b 第3次 段階1) |
| `RUN_V3_BASELINE` | `"false"` | v3 baseline の hardware baseline (`build_v3_rx72n -> flash/download -> provision -> MQTT`) のみを実行するか (段階2-3) |
| `RUN_V3_OTA` | `"false"` | v3 baseline の OTA 再検証 (`build_v3_ota_rx72n -> flash_v3_boot_ota_rx72n -> download_v3_v1_ota_rx72n -> provision_v3_ota_rx72n -> ota_v3_create_job_rx72n -> ota_v3_monitor_rx72n -> ota_v3_finalize_rx72n` の 7 jobs) のみを実行するか (段階4)。**注意**: ENVK 1台で連続実行する際は AWS 側の job 重複を避けるため `RUN_AWS_TESTS=false` 併用が安全 |
...
`RUN_V3_BUILD_ONLY=true` を指定すると、legacy `aws_demos` / OTA / 実機ジョブをすべて止め、
Windows runner 上の `build_v3_rx72n` だけを実行する (Phase 8b 第3次 段階1, MR !77)。
v3 baseline の retarget 作業を scarce な実機資源を消費せずに回すためのモード。

`RUN_V3_BASELINE=true` を指定すると、legacy 系 job を止め、
`build_v3_rx72n -> flash/provision/MQTT` を実行する (段階2-3, MR !79 / !82)。
4/4 markers (IP Address / MQTT Receive / Topic Subscribe / MQTT Publish) PASS を Pipeline #3006 で確認済み。

`RUN_V3_OTA=true` を指定すると、legacy 系 job を止め、
`build_v3_ota_rx72n -> flash_v3_boot_ota_rx72n -> download_v3_v1_ota_rx72n -> provision_v3_ota_rx72n -> ota_v3_create_job_rx72n -> ota_v3_monitor_rx72n -> ota_v3_finalize_rx72n` の 7 jobs を実行する
(段階4, MR !83)。Pipeline #3123 で全 7 jobs success。最新 FreeRTOS baseline 上で OTA pipeline を再検証するためのモード。
...
- `RUN_V3_OTA` は v3 baseline (iot-reference-rx HEAD 同期) の OTA demo を前提とした別系統の OTA パイプライン。v1/v2 RSU は `tools/ci/build_v3_rx72n_ota.ps1` で生成し、`tools/v3/build_fwup_v2_rsu.py`（"Renesas" magic, legacy boot_loader 互換）で RSU 化する。**RSU builder 注意**: `tools/build_fwup_v2_rsu.py` (RELFWV2 magic, r_fwup sample boot_loader 用) と混同しないこと。boot_loader_rx72n_envision_kit は legacy boot_loader を再利用しているため "Renesas" format が必須
...
**Raspberry Pi 側 USB 配線マージン仮説（未確定だが重要）:**

前セッションの現場観測では、Raspberry Pi #1 と #2/#3 で USB host 周りの個体差が疑われている。具体的には、ボード製造年差に起因する USB 関連チップセット差や電源供給余裕の差により、RX72N Envision Kit 側の消費電流が増える局面で不安定化している可能性がある、という仮説が出ている。

同じ観測の中で、Raspberry Pi と RX72N Envision Kit の間にある USB 配線 3 本を USB ハブ経由から直結へ変更したあと、初めて full pipeline 成功 (`#784`, 2026-03-14 16:07 JST 開始) が得られた、という履歴がある。一方で、その後も `#786`、`#814`、`#815` などの失敗が出ているため、直結化は「電気的マージン改善の可能性が高い有力な対症策」ではあるが、「それだけで不安定化を解消する十分条件」とまではまだ確認できていない。

2026-03-15 時点の整理では、`#814/#815` や `#834/#835` の一部は cross-pipeline 干渉で説明できるため、これらは USB 直結の有効性を評価する材料には使いにくい。USB ハブ経由 vs 直結の効果を本当に確かめるには、同一 `DEVICE_ID`・同一 isolated pipeline 条件での A/B 再現が必要である。
...
1. **OTA activation hang (`decodedData` stack overflow)**: `OtaOverMqttDemo.c:1318` の `uint8_t decodedData[4096]` がスタック割当だったため、448 blocks download + Close file 後の Activate Image が出ず silence。`static uint8_t decodedData[mqttFileDownloader_CONFIG_BLOCK_SIZE] = {0}` で BSS に移動して解消。iot-reference-rx fork commit `b457f4fe` には block size 8192 + frame buffer shift も含まれていたが、8192 は MQTT バッファ超過で Dumped packet 発生のため 4096 に据え置き
2. **`ota_pal.c` 未同期 (import 後の 7 commit)**: decodedData fix だけでは activation hang 解消せず。iot-reference-rx import (`11533b11`) 後に OTA-critical な 7 commit (`6764ccb1` flash writes restructure 890行 / `337fa334` persist OTA image state across reboot / `935df02f` use bootloader header layout / `72f5e945` harden flash queue bounds 他) が追加されていた。iot-reference-rx HEAD (`023e13ea`) の `ota_pal.c` + `store.h` を丸ごと sync。ただし `tcp_sockets_wrapper.c` の 32KB buffer は revert（heap 枯渇でネットワーク死亡のため）
3. **RSU builder 混同**: `tools/build_fwup_v2_rsu.py` = `RELFWV2` magic (r_fwup sample boot_loader 用), `tools/v3/build_fwup_v2_rsu.py` = "Renesas" magic (legacy boot_loader 用) が正解。`boot_loader_rx72n_envision_kit` は legacy boot_loader を再利用しているため "Renesas" format が必須
4. **Monitor post-reboot UART 検出**: OTA 完了後の bank swap で 3 回 reboot → UART 接続が切れる。monitor STEP 3 に `--reset-cmd` 追加し、明示 reset 後に version 検出するよう修正
5. **CN8 撤去影響**: 2026-04-17 に全 RX72N の CN8 (RL78/G1C) USB ケーブル撤去。`COMMAND_PORT` を CN6 (SCI7 / FTDI / 921600bps) に統一し、Windows workflow 分岐を削除。Legacy 4 jobs (`test_commands` / `test_screen_navigation` / `provision_aws_credentials` / `confirm_aws_mqtt`) は SCI2 前提のため `allow_failure: true` に降格（後段で SCI7 対応へ改造予定）

ハードウェア知見:

- E2OB (OBE110008) deep hung が 2 回発生。SSH USB unbind/rebind 無効、物理電源 ON/OFF でのみ復旧。仮説は「boot_loader 起動失敗 → セルフリセット 1 秒周期ループが E2 Lite に USB ストレスを与え固着誘発」（memory `project_e2ob_reset_loop_hypothesis.md`）
- test farm 拡張に向けてリモート電源制御（スマートプラグ等）の runner 要件織り込みが必要（memory `project_remote_power_control_required.md`）
...
同じ overlap pattern は full pipeline の `#814` と `#815` にも出ている。`#814` は `v1=930 / v2=931` を build し、stale match なしで OTA update `rx72n-ota-1773487419` を作成した。一方その数分後に始まった `#815` は同じ slot #1 に対して `v1=932 / v2=933` を build し、自分の `ota_create_job` で `rx72n-ota-1773487419` を明示的に match/deleted したうえで `rx72n-ota-1773487560` を作成している。決定的なのは `#814` の `ota_monitor` で、期待 build は `931` なのに monitor 開始時点の current firmware がすでに `2.0.932` だった。これは `#814` が monitor に入る前に、後発 `#815` の `prepare_ota` が board を書き換えていたことを意味する。`#834/#835` と同様、同じ device / Thing name / 固定 S3 key `ota/rx72n-01/ota_v2.rsu` を overlapping pipeline が共有していたのが本質である。

ただし、これで全ての unstable run が説明できるわけではない。pipeline `#786` は同じ overlap signature を持っておらず、`#784` は `#786` 開始前にすでに完了していたし、`#786` の boundary window 中に slot #1 へ重なる legacy OTA pipeline も見えていない。したがって現時点の見取り図は二層で、`#814/#815` と `#834/#835` は cross-pipeline 干渉で説明できる一方、`#786` は依然として clean な non-overlap failure として残っており、runtime / hardware / reset 側の本丸はこちらで追うべきである。

実務上の含意は明確で、現行 lock は legacy OTA 解析には粒度が粗い。`resource_group` と `acquire_pi_device_lock.sh` は Raspberry Pi job 単位では排他するが、`prepare_ota -> ota_create_job -> ota_monitor -> ota_finalize` の境界全体を予約していない。そのため先行 pipeline が monitor に入る前に、後続 pipeline が board を reflash したり AWS OTA update を delete/replace したりできてしまう。Issue `#18` の今後の repro では、同じ `DEVICE_ID` / Thing name を触る concurrent pipeline を避けるか、少なくとも OTA boundary 全体を跨ぐ reservation mechanism を導入する必要がある。
...
### 2026-03-09: `phase8b/` へ seed import を実施
...
### 2026-03-09: Phase 8b 開始 - skeleton import 計画と issue 分割
...
```
docs/
├── README.md          # トップレベル目次 (en/ja へのリンク)
├── en/                # 英語ドキュメント
│   ├── README.md      # 英語目次 (Wiki Home 由来)
│   ├── quick-start/
│   ├── features/
│   ├── developer/
│   ├── bare-metal/
...
| 英語ページ | 26 |
| 日本語ページ | 30 |
| スキップ (stub/meta) | 4 (`Quick-Start-Guide.md`=dummy, `1-Ether-TCP-IP-Web-Server_.md`=Now Editing, `_Footer.md`, `_Sidebar.md`) |
| 存在しないリンク (英語ページ由来) | 3 (`D2-audio.md`, `MEMS-mic.md`, `ESP32.md`) |
| 画像・データファイル | `data/` 配下 117 ファイル |
| 変換先カテゴリ | 6 (`quick-start`, `features`, `developer`, `bare-metal`, `freertos`, `tools`) |

**移行スクリプト: `migrate_wiki.py`**
...
| MR | 内容 | 変更ファイル |
|---|---|---|
| !1 | 初回 Wiki 移行 (58ページ + 117画像) | 約180ファイル |
| !2 | GitHub アカウント名修正、CLAUDE.md へ project goals 追加 | 2ファイル |
| !3 | broken relative links 修正 (33ファイル)、CLAUDE.md status update | 33ファイル |
