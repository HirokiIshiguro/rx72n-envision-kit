...
| 3 | UART ダウンロード後のフリーズ現象の解析（2回に1回、boot_loader → aws_demos 転送後に画面表示なしでフリーズ） | Planned |
...
- **j24 並列ビルドでレースコンディション発生の可能性あり。** e2studio ヘッドレスビルドで aws_demos が `Error 1` で失敗する場合、ccrx 単体では成功する。回避策: e2studio で Makefile 生成後、CLI ビルド（make 直接実行）にフォールバック
...
rfp-cli -device RX72x -tool "e2l:<serial>" -if fine -speed 1500K \
  -auth id FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF -e -p <mot_file> -run -noquery
```
...
- **`-run` は全操作で必須。** rfp-cli 接続時に E2 Lite が MCU をリセット・停止させるため、`-run` なしで切断すると MCU がホールド状態のまま停止する（LCD 消灯、UART 無応答）。読み取り専用操作（`-rv`, `-sig` 等）でも末尾に `-run` を付けること

| モード | 速度 | 所要時間 | 備考 |
|--------|------|----------|------|
| boot_loader 単体 `-auto` | 500K | 約13秒 | CI/CD パイプライン標準 |
...
- `testing` (0xFE): boot_loader が integrity check → bank swap を実行（フリーズ現象の再現テスト用）
...
### 2026-03-01: ビルド手順・Flash 設定の改善

Updated CLAUDE.md with build and flash improvements based on manual test session:
- e2studio headless build j24 race condition documented (aws_demos `Error 1` with ccrx succeeding standalone; workaround: fall back to CLI make)
- rfp-cli speed optimized: 500K → 1500K (max supported), verify skip option (`-e -p` instead of `-auto`)
- Factory MOT 1-command flash procedure added (122s vs 396s with old settings = 69% faster)
- Speed comparison table added
- `--image-flag` options documented (valid for normal boot, testing for freeze reproduction)

手動テストセッションの結果に基づき CLAUDE.md のビルド・Flash 手順を改善:
- e2studio ヘッドレスビルドの j24 レースコンディションを記載（aws_demos が Error 1 で失敗、ccrx 単体では成功。CLI make へのフォールバックで回避）
- rfp-cli 速度を最適化: 500K → 1500K（最大対応速度）、Verify 省略オプション追加
- factory MOT による 1 コマンド書き込み手順を追加（122秒 vs 旧設定 396秒 = 69% 短縮）
- 速度比較テーブルを追加
- `--image-flag` オプション（valid: 通常起動、testing: フリーズ再現テスト）を記載
