...
/**********************************************************************************************************************
 * Function Name: SocketErrorHook
 * Description  : Handles cellular module errors by performing hardware reset and reconnection to AP based on
 *                error type and force_reset flag.
 * Arguments    : error        The cellular error code to be handled
 *              : force_reset  Flag to force immediate hardware reset (true: force, false: conditional)
 * Return Value : error        The cellular error code passed in
 *                0            Communication error below threshold, no action taken.
 *********************************************************************************************************************/
e_cellular_err_t SocketErrorHook(e_cellular_err_t error, bool force_reset)
{
...
/**********************************************************************************************************************
 * Function Name: Connect2AP
 * Description  : Establishes cellular module connection to Access Point with automatic retry and hardware reset
 *                on failure.
 * Arguments    : None
 * Return Value : true   Successfully connected to AP or already connected.
 *                false  Connection failed after maximum retry attempts.
 *********************************************************************************************************************/
bool Connect2AP(void)
{
    uint32_t         countAP = 0;
    ...
    do
    {
        ...
        if ((CELLULAR_SUCCESS == ret) || (CELLULAR_ERR_ALREADY_CONNECT == ret))
        {
            LogInfo(("Connected to AccessPoint \r\n "));
            return ((CELLULAR_SUCCESS == ret) || (CELLULAR_ERR_ALREADY_CONNECT == ret));
        }
        else
        {
 ...
 * Description  : Closes the specified cellular socket with automatic retry on failure.
 * Arguments    : socket_number  The socket number to be closed
 * Return Value : None
 *********************************************************************************************************************/
void CloseSocket(uint32_t socket_number)
{
    ...
    else
    {
        ...
        /**************************
         * The following process is executed if all retry attempts of `R_CELLULAR_CloseSocket` has exhausted without success.
         * Please enable and modify these code if necessary.
         ****************************/
        /*
