...
static void sci_callback(void *pArgs)
{
    ...
    /* The pArgs should be cast to the pointer to sci_cb_args_t */
...
static void sample_buffering(uint8_t rx_data)
{
    ...
    {
        /* end of file? */
        if ((0 < s_file_size) && (s_file_size <= s_flash_buf.total))
        {
            BL_UART_RTS = 1;
        }
    }
...
#if (BL_ENABLE_ROLLBACK == 1)
    /* If signature verification fails and there is image on the buffer area then swap the bank */
    else if (true == R_FWUP_IsExistImage(FWUP_AREA_BUFFER))
    {
        sample_activate_img();
    }
    else
