...
アーキテクチャ:
  ...
  E2 Lite と COM6 は同一ボード上の USB ハブを共有しているため、
  rfp-cli がデバッガを切断する際に USB バスが一時的にリセットされ、
  先に開いていた COM ポートが無効化される可能性がある。
  flash 完了 → USB 安定化待ち → COM open → read の順序で回避する。
...
def main():
    ...
    if args.flash_cmd:
        ...
        # USB 安定化待ち
        # E2 Lite と COM6 が同一 USB ハブ上にあるため、
        # rfp-cli のデバッガ切断後に USB バスが再安定するのを待つ
