...
def create_ota_job(thing_name, s3_bucket, s3_key, s3_version_id,
                   ota_role_arn, region, account_id):
    """AWS IoT OTA ジョブを作成する。

    カスタム署名方式: .rsu は mot_to_rsu.py で ECDSA-SHA256 署名済み。
    AWS Signer は使用せず、existingSignature + customCodeSigning で指定。
    """
    ...
    ota_files = [{
        ...
        "codeSigning": {
            "customCodeSigning": {
                "signature": {
                    "inlineDocument": "ALREADY_SIGNED_IN_RSU"
                },
                "certificateChain": {
...
def main():
    ...
    try:
        ...
        ota_update_id = create_ota_job(
            ...
            args.ota_role_arn, args.region, account_id
        )
