...
function Get-BoardAppDefines {
    ...
    foreach ($rawToken in ($TaskList -split '[,; ]+')) {
        ...
        switch ($token) {
            ...
            "gui_lcd_diag" { $defines += "appmainENABLE_BOARD_GUI_TASK=1"; $defines += "appmainENABLE_BOARD_GUI_LCD_DIAGNOSTIC_TASK=1"; break }
            "gui_setup_only" { $defines += "appmainENABLE_BOARD_GUI_TASK=1"; $defines += "appmainENABLE_BOARD_GUI_SETUP_ONLY_TASK=1"; break }
            "gui_core_init_only" { $defines += "appmainENABLE_BOARD_GUI_TASK=1"; $defines += "appmainENABLE_BOARD_GUI_CORE_INIT_ONLY_TASK=1"; break }
            ...
            { $_ -in @("no_ota", "ota_off", "disable_ota") } { $defines += "ENABLE_OTA_UPDATE_DEMO=0"; break }
            { $_ -in @("no_ota_task", "skip_ota_task", "pubsub_no_ota_wait", "skip_ota_wait") } { $defines += "appmainSTART_OTA_DEMO_TASK=0"; $defines += "mqttexampleSKIP_OTA_SELF_TEST_WAIT=1"; break }
            { $_ -in @("pubsub_skip_ota_wait", "skip_pubsub_ota_wait") } { $defines += "mqttexampleSKIP_OTA_SELF_TEST_WAIT=1"; break }
            { $_ -in @("keep_cli", "cli_diag", "diagnostic_cli") } { $defines += "appmainKEEP_CLI_TASK_FOR_DIAGNOSTICS=1"; $defines += "configINCLUDE_QUERY_HEAP_COMMAND=1"; break }
            ...
            { $_ -in @("no_glcdc_pinset", "gui_no_glcdc_pinset") } { $defines += "appmainDISABLE_EMWIN_GLCDC_PINSET=1"; break }
            { $_ -in @("no_emwin_dmaca_init", "gui_no_emwin_dmaca_init") } { $defines += "appmainDISABLE_EMWIN_DMACA_INIT=1"; break }
            { $_ -in @("no_glcdc_open", "gui_no_glcdc_open") } { $defines += "appmainDISABLE_EMWIN_GLCDC_OPEN=1"; break }
            { $_ -in @("no_glcdc_gr2_read", "gui_no_glcdc_gr2_read") } { $defines += "appmainDISABLE_EMWIN_GLCDC_GR2_READ=1"; break }
            { $_ -in @("random_probe", "tcp_random_probe") } { $defines += "appmainENABLE_TCP_SOCKET_RANDOM_PROBE=1"; break }
            { $_ -in @("fast_random", "deterministic_random", "insecure_random_probe") } { $defines += "appmainUSE_DETERMINISTIC_RANDOM_PROBE=1"; break }
            ...
            default        { throw "Unknown RX72N_BOARD_APP_TASKS token '$rawToken'. Use none, all, gui, gui_stub, gui_lcd_diag, gui_setup_only, gui_core_init_only, gui_no_root, gui_init_only, gui_persistent, no_gui_persistent, gui_post_root_probe, gui_force_root_fullscreen, no_gui_force_root, gui_post_root_draw, gui_generated_loop, gui_manual_loop, no_trace, no_ota, no_ota_task, pubsub_skip_ota_wait, keep_cli, no_tcp_perf, no_gui_timer, no_gui_touch, no_glcdc_vpos, edmac_first, no_glcdc_pinset, no_emwin_dmaca_init, no_glcdc_open, no_glcdc_gr2_read, random_probe, fast_random, no_appw_multibuf, sdcard, serial_flash, audio." }
        }
    }
