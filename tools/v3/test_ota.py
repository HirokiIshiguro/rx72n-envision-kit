...
# --- マイルストーン定義 (v3: iot-reference-rx canonical) ---
# v3 の OTA デモは Demos/OtaOverMqtt/OtaOverMqttDemo.c で実装されており、
# AWS IoT OTA library + coreMQTT Agent + Jobs/MQTT file streams の構成。
# ログ文言は OtaOverMqttDemo.c の LogInfo() 呼び出しから採取。
OTA_MILESTONES = {
    "agent_ready": {
    ...
    "job_received": {
        ...
        "patterns": [
            r"Received OTA Job\.",
            r"Received Job Document event Received",
            r"Request Job Document event Received",
        ],
        "required": True,
    },
    "download_started": {
        ...
        "patterns": [
            r"Starting The Download\.",
            r"Request File Block event Received",
        ],
        "required": True,
    },
    "download_complete": {
        "description": "All blocks received",
        "patterns": [
            r"Downloaded block \d+ of \d+",
        ],
        "required": True,
    },
    "self_test": {
        "description": "Self-test mode entered after reboot",
        "patterns": [
            r"Image self-test passed!",
        ],
        "required": False,
    },
    "accepted": {
        "description": "New image accepted (higher version)",
        "patterns": [
            r"New image has higher version than current image, accepted!",
            r"OTA Completed successfully!",
        ],
        "required": True,
    },
}
...
def monitor_ota_progress(ser, timeout=600, port_label="UART"):
    ...
    if "download_complete" in detected and "accepted" not in detected:
        stall_hint = "stalled_after_download_complete"
        print("[WARN] OTA appears stalled after download completion")
    elif "job_received" in detected and "download_started" not in detected:
        stall_hint = "stalled_after_job_received"
        print("[WARN] OTA appears stalled after job received")
